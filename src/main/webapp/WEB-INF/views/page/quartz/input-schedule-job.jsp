...
<form action="save-schedule-job" method="post" class="form-horizontal">
    ...
    <div class="form-group">
        ...
        <div class="col-sm-8">
            <input type="radio" name="isSync" value="false" <c:if test="${scheduleJobVo.isSync == 'false'}">checked</c:if> />同步
            <input type="radio" name="isSync" value="true" <c:if test="${scheduleJobVo.isSync == 'true'}">checked</c:if> />异步
        </div>
    </div>
